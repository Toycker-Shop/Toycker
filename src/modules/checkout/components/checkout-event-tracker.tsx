...
export default function CheckoutEventTracker({ cart }: { cart: Cart }) {
  const trackedCartId = useRef<string | null>(null)

  useEffect(() => {
    if (trackedCartId.current === cart.id) return
    const items: MarketingItem[] = (cart.items ?? [])
    ...
    trackCartEvent("begin_checkout", items, cart.total ?? cart.subtotal ?? 0, cart.currency_code.toUpperCase())
  }, [cart])
