...
export default function VisualSearchInterface() {
  ...
  const performSearch = useCallback(
    async (forceFullImage = false, cropOverride?: PixelCrop) => {
      ...
      try {
        ...
        if (!response.ok) {
          ...
          if (
            data.error?.includes("PGRST202") ||
            data.error?.includes("search_products_multimodal")
          ) {
            throw new Error(
              "Database setup incomplete. Please run the SQL migration for multimodal search."
            )
          }
        ...
        <div className="max-w-md w-full text-center space-y-8">
          <div className="space-y-4">
            ...
            <p className="text-slate-500 text-lg">
              Can&apos;t find the right words? Upload a photo to find matching
              toys instantly.
            </p>
          </div>
          ...
          <div className="pt-4">
            <div className="flex flex-col gap-3 sm:flex-row sm:justify-center">
              <button
                ...
                className="w-full sm:w-auto px-8 py-4 bg-primary text-white text-lg font-bold rounded-2xl shadow-xl shadow-primary/20 hover:bg-primary/90 hover:-translate-y-0.5 transition-all duration-300 flex items-center justify-center gap-3"
              >
              ...
              <button
                ...
                className="w-full sm:w-auto px-8 py-4 bg-white border-2 border-primary/20 text-primary text-lg font-bold rounded-2xl hover:bg-primary/5 hover:border-primary/40 transition-all duration-300 flex items-center justify-center gap-3"
              >
            ...
            <div className="flex items-center justify-between mb-6">
              <div>
                ...
                <p className="text-slate-500 text-sm mt-1">
                  Adjust the selection to focus on a specific toy for better
                  results.
                </p>
              </div>
            </div>
            ...
            <div className="mt-6">
              ...
              <div className="flex flex-col gap-3 sm:flex-row">
                ...
                <button
                  ...
                  className="w-full flex items-center justify-center gap-2.5 px-6 py-4 bg-white border-2 border-primary/20 rounded-2xl text-primary font-bold hover:bg-primary/5 hover:border-primary/40 transition-all group"
                >
                  <PhotoIcon className="w-6 h-6 transition-transform group-hover:scale-110" />
                  Choose from Gallery
                </button>
              </div>
            </div>
        ...
        <div className="w-full lg:w-3/4">
          ...
          {!hasInteracted && (
            <div className="flex flex-col items-center justify-center py-32 text-center opacity-40">
              ...
              <p className="text-lg">
                Select a toy in the image to start searching
              </p>
            </div>
          )}
            ...
            <div className="flex flex-col items-center justify-center py-32 text-center">
              <div className="rounded-2xl border border-dashed border-slate-200 bg-white p-12 w-full">
                ...
                <p className="mt-2 text-sm text-slate-500">
                  Try selecting a different part of the image or different
                  lighting.
                </p>
              </div>
            </div>
